...
# docker run
/usr/local/bin/docker-entrypoint.sh docker run --rm --name $HOSTNAME --user root -v /var/tmp:/var/tmp -v /etc/localtime:/etc/localtime:ro --privileged $ENVS $NETWORKS $BINDS "$@" $VCAC_IMAGE &
